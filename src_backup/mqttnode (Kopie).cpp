...
//    fclose(file);
    ...
    if(m_instanz)
    {
        m_instanz->setLastMessage(topicName,payloadbuffer);
//        m_instanz->setLastTopicReceived(topicName);
    }
//    printf("%s\n",payloadbuffer);
...
void MQTTNode::setLastMessage(char* t, char *m)
{
    strcpy(m_lastMessageReceived,m);
    strcpy(m_lastTopicReceived,t);
    benachrichtige();
}
...
char *MQTTNode::getLastMessageReceived(char *m)
{
    if(m)
    {
        strcpy(m,m_lastMessageReceived);
    }
    return m;
}

char *MQTTNode::getLastTopicReceived(char *t)
{
    if(t)
    {
        strcpy(t,m_lastTopicReceived);
    }
    return t;
}

MQTTNode* MQTTNode::getInstanz()
{
    if(m_instanz)
        return m_instanz;
    else
        return (m_instanz = new MQTTNode());
}
